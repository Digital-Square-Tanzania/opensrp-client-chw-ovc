...
public class OvcJsonFormUtils extends org.smartregister.util.JsonFormUtils {
    ...
    public static Event processJsonForm(AllSharedPreferences allSharedPreferences, String
        ...
        if (Constants.EVENT_TYPE.OVC_REGISTRATION.equals(encounter_type)) {
            encounter_type = Constants.TABLES.OVC_REGISTER;
        } else if (Constants.EVENT_TYPE.MVC_HOUSEHOLD_SERVICES_VISIT.equals(encounter_type) || Constants.EVENT_TYPE.MVC_SERVICES_VISIT.equals(encounter_type)) {
    ...
    protected static FormTag formTag(AllSharedPreferences allSharedPreferences) {
        ...
        formTag.appVersion = OvcLibrary.getInstance().getApplicationVersion();
        formTag.databaseVersion = OvcLibrary.getInstance().getDatabaseVersion();
        return formTag;
    }
    ...
    public static void tagEvent(AllSharedPreferences allSharedPreferences, Event event) {
        ...
        event.setClientApplicationVersion(OvcLibrary.getInstance().getApplicationVersion());
        event.setClientDatabaseVersion(OvcLibrary.getInstance().getDatabaseVersion());
    }
    ...
    public static JSONObject getFormAsJson(String formName) throws Exception {
        return FormUtils.getInstance(OvcLibrary.getInstance().context().applicationContext()).getFormJson(formName);
    }
