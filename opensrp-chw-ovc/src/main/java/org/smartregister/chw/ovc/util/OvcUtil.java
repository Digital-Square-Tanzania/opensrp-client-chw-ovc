...
public class OvcUtil {

    public static void processEvent(AllSharedPreferences allSharedPreferences, Event baseEvent) throws Exception {
        if (baseEvent != null) {
            OvcJsonFormUtils.tagEvent(allSharedPreferences, baseEvent);
            JSONObject eventJson = new JSONObject(OvcJsonFormUtils.gson.toJson(baseEvent));
    ...
    public static boolean launchDialer(final Activity activity, final BaseOvcCallDialogContract.View callView, final String phoneNumber) {
        if (ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {

            // set a pending call execution request
            if (callView != null) {
                callView.setPendingCallRequest(() -> OvcUtil.launchDialer(activity, callView, phoneNumber));
            }
    ...
    public static void saveFormEvent(final String jsonString) throws Exception {
        ...
        Event baseEvent = OvcJsonFormUtils.processJsonForm(allSharedPreferences, jsonString);
        OvcUtil.processEvent(allSharedPreferences, baseEvent);
    }
